...
This folder ships in the repo (there is no packaging step for an Avalon PWA monorepo — the SvelteKit build pulls only from `packages/web/`, so `backlog/` never ends up in the artifact). Treat it as maintainer-facing knowledge, not user-facing docs.
