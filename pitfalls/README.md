...
This folder ships in the repo (there is no packaging step for an Avalon PWA monorepo — the SvelteKit build pulls only from `packages/web/`, so `pitfalls/` never ends up in the artifact). Treat it as maintainer-facing knowledge, not user-facing docs.
...
|---|---|---|
| [playwright-getbytext-strict-mode-multiple-matches](playwright-getbytext-strict-mode-multiple-matches.md) | `strict mode violation: getByText(...) resolved to 2 elements` | workaround documented |
| [svelte5-effect-infinite-loop-on-state-assignment](svelte5-effect-infinite-loop-on-state-assignment.md) | `WebSocket failed: Insufficient resources` · effect re-runs on `$state` assignment | workaround documented |
| [vite-websocket-proxy-econnreset-with-bun-serve](vite-websocket-proxy-econnreset-with-bun-serve.md) | `[vite] ws proxy socket error: read ECONNRESET` · Vite proxy to Bun.serve | workaround documented |
