...
<script lang="ts">
  ...
  const roleName = $derived(t(`role.name.${role}`));
...
<div class="text-center">
  <p class="font-display text-xs tracking-[0.25em] opacity-60 uppercase">{t('role.yourRole')}</p>
  ...
  <p class="mt-1 text-xs tracking-wider opacity-60 uppercase">
    {t(alignment === 'evil' ? 'role.alignment.evil' : 'role.alignment.good')}
  </p>

  {#if seenList.length > 0}
    <div class="mt-4 rounded-lg border border-ink/15 bg-parchment-deep/40 p-3 text-left">
      <p class="mb-1 text-xs font-medium tracking-wider opacity-70 uppercase">
        {t('role.youSee')}
      </p>
        ...
        {#each seenList as s (s.pid)}
          <li class="flex items-baseline justify-between gap-2">
            ...
            <span
              ...
              class:bg-blood={s.kind === 'evil'}
              class:text-parchment={s.kind === 'evil' || s.kind === 'merlin-like'}
              class:bg-gold={s.kind === 'merlin-like'}
              class:bg-good={s.kind === 'good'}>{s.label}</span
            >
          </li>
        {/each}
      </ul>
