...
{#if !view}
  <div class="mt-12 text-center">
    <p class="font-display text-lg tracking-wide opacity-80">
      Connecting to <strong>{roomLabel}</strong>…
    </p>
    <p class="mt-2 text-xs opacity-60">Connection: {session.state}</p>
  </div>
{:else}
  ...
  <div class="grid gap-4 lg:grid-cols-[1fr_minmax(0,360px)]">
    ...
    <div class="space-y-4">
      ...
      <Card class="border-gold/30 bg-parchment-deep/30">
        ...
        {:else if view.phase === 'team_selection'}
          ...
          {#if iAmCaptain}
            <p class="text-sm opacity-70">
              Pick exactly {view.teamSizeRequired} players. Selected:
        ...
        {:else if view.phase === 'lady_of_lake'}
          <h2 class="font-display mb-2 text-xl font-bold">🌊 Lady of the Lake</h2>
          {#if iHoldLady}
            <p class="text-sm">
    ...
    <aside class="space-y-4">
      ...
      <Card class="!p-0">
        ...
        >
          <span class="opacity-70">Chat · {store.chat.length}</span>
